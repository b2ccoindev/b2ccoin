...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete B2CCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no B2CCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old B2CCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected B2CCoin 0.7 datadir."
    ;;
  3)
    echo "Detected B2CCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected B2CCoin 0.8 datadir."
    ;;
esac
