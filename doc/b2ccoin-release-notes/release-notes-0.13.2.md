B2CCoin Core version 0.13.2 is now available from:

  <https://download.b2ccoin.org/b2ccoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/b2ccoin-project/b2ccoin/issues>
...
updates will be released anymore. Without security updates, using a b2ccoin
...
ECDSA signatures inside B2CCoin transactions now use validation using
...
Previous versions of B2CCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of B2CCoin Core was to raise the default minimum
relay fee.

B2CCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
B2CCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running b2ccoind without having to do any manual configuration.
...
B2CCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. B2CCoin Core will relay transactions with
...
entirely in B2CCoin Core 0.13, and it is at this time undecided whether the
...
B2CCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
B2CCoin Core automatically creates a hidden service to listen on, without
manual configuration. B2CCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if B2CCoin Core is listening, and
...
B2CCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals B2CCoin
Core to use floating fees. By default, B2CCoin Core will use floating
fees.
...
(default: `0.0002` B2C/kB).

At all times, B2CCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) B2C.
Furthermore, B2CCoin Core will never create transactions paying less than
...
`b2ccoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /b2ccoinrpc>
        ...
        # optional bypass b2ccoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< b2ccoinrpc:<password>
    </Location>
...
Restarting b2ccoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in b2ccoin.conf
...
For example:
    ...
    $ src/b2ccoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The B2CCoin Core code base has
...
- `b2ccoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `b2ccoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the B2CCoin scripting system that in combination with
...
B2CCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of B2CCoin Core.
...
The two phases are distinct in the B2CCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming B2CCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for B2CCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each B2CCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of b2ccoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of b2ccoins being spent is exactly
...
- **Increased security for multisig:** B2CCoin addresses (both P2PKH addresses
  ...
  believed to be provided by B2CCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of B2CCoin's current ledger (called the UTXO set),
  ...
  note that B2CCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  B2CCoin users to individually opt-in to almost any change in the B2CCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and B2CCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the B2CCoin
client to B2CCoin Core 0.13.2 to signal for segwit support. When a
...
Since B2CCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `b2ccoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `b2ccoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
