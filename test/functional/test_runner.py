...
def main():
    ...
    tmpdir = "%s/b2ccoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and b2ccoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "b2ccoind"]) is not None:
            print("%sWARNING!%s There is already a b2ccoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "B2CCOIND" not in os.environ:
        os.environ["B2CCOIND"] = build_dir + '/src/b2ccoind' + exeext
        os.environ["B2CCOINCLI"] = build_dir + '/src/b2ccoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `b2ccoin-cli help` (`rpc_interface.txt`).
