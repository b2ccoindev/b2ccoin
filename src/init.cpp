...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/b2ccoin-project/b2ccoin>";
    const std::string URL_WEBSITE = "<https://b2ccoin.org>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if b2ccoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if b2ccoin is started while in a temporary directory.\n",
